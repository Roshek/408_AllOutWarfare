...
waituntil {triggeractivated _basActivated};
	...
	_mkr setmarkeralpha _mAH;
						...
						_bastStarted = createTrigger ["EmptyDetector",_mPos];
						_bastStarted setTriggerArea [(_mkrX+(_Placement*0.3)),(_mkrY+(_Placement*0.3)),_mkrAgl,FALSE];
						_bastStarted setTriggerActivation [_enemyFaction,"PRESENT",true];
						_bastStarted setTriggerTimeout [1, 1, 1, true];
						_bastStarted setTriggerStatements ["this","",""];
...
// SPAWN PATROLS
	...
	for "_counter" from 1 to _PApatrols do {
						_pos = [_mPos, _Placement, random 360] call BIS_fnc_relPos;
							...
							0=[_grp,"INFskill"] call eos_fnc_grouphandlers;
...
//SPAWN LIGHT VEHICLES
	...
	for "_counter" from 1 to _LVehGroups do {
		...
		_newpos = [_mPos, (_Placement +250), random 360] call BIS_fnc_relPos;
...
//SPAWN HELICOPTERS
    ...
    if ((_fSize select 0) > 0) then {_vehType=4}else{_vehType=3};
				_newpos = [_mPos, (_Placement +1000), random 360] call BIS_fnc_relPos;
    ...
    if ((_fSize select 0) > 0) then {
		...
		0=[(_fGroup select 2),"INFskill"] call eos_fnc_grouphandlers;
...
waitUntil {triggerActivated _bastStarted};

_waveOngoing = true;
while {_waveOngoing} do {
	sleep 1;

	if (!triggeractivated _bastActive || getmarkercolor _mkr == "colorblack") exitwith
			{
					["Wave failed","hintSilent",nil,false] call BIS_fnc_MP;
	...
	if (triggeractivated _bastActive and triggeractivated _bastClear) then
			{
			if (_hints) then  {["Wave completed","hintSilent",nil,false] call BIS_fnc_MP;};
					_mkr setmarkercolor VictoryColor;
					_mkr setmarkeralpha _mAN;
					_waveOngoing = false;
				};
...
waituntil {getmarkercolor _mkr == "colorblack" OR getmarkercolor _mkr == VictoryColor OR getmarkercolor _mkr == hostileColor or !triggeractivated  _bastActive};
	...
	deletevehicle _bastActive;deletevehicle _bastClear;deletevehicle _basActivated; deletevehicle _bastStarted;
