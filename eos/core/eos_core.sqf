				...
				_units=_eosActivated getvariable _cacheGrp;
					...
					_bMin=_bSize select 0;
							...
							if (_bMin > 0) then {
								...
								if (_faction == 3) then {
									0=[_bGroup,"INFskill",1] call eos_fnc_grouphandlers;

								} else {
								0=[_bGroup,"INFskill"] call eos_fnc_grouphandlers;
								if (_ups == 1 && _side != CIVILIAN) then {
									nul=[leader _bGroup,_mkr] execVM "scripts\ups.sqf";
								} else {0 = [_bGroup,_mkr] call EOS_fnc_taskpatrol;};
												};};
		};
...
if (typename _cGroup != "SCALAR") then {
...
if (typename _dGroup != "SCALAR") then {
...
if (typename _fGroup != "SCALAR") then {
