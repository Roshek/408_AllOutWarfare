...

{
    ...
    if (_zombie == 1) then {
    _infect = _unit spawn INF_fnc_infecthim;
    _unit addEventHandler ["HandleDamage",{if (((_this select 1) in ["head"]) && (_this select 2 >= 0.8)) then {(_this select 0) setDamage 1;}else{(_this select 0) setDamage 0;}; if (alive (_this select 0) && (_this select 1) in ["legs"] && (_this select 2) >= 2) then {(_this select 0) setUnitPos "DOWN"}}];
